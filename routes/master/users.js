   ...
   try {
      ...
      if (decode.logged) {
         ...
         if (result[0][0].status === "ok") {
            res.end(JSON.stringify({
               success: true,
               message: `Pengguna "${form.name}" berhasil dibuat`,
            }))
         } else {
            res.end(JSON.stringify({
               success: false,
...
exports.update = async (req, res) => {
   ...
   try {
      ...
      if (decode.logged) {
         db = await conn.getConnection()
         const query = `call usersUpdate(?,?,?,?,?,?,?,?,?,?,?,?)`
         const result = await db.query(query, [            
            form.id,
            form.name,
            form.newPassword && form.password !== form.newPassword ? enc(form.newPassword) : form.password,
            form.email,
            form.phone,
            form.fullname,
            form.privileges || [],
         ...
         if (result[0][0].status === "ok") {
            res.end(JSON.stringify({
               success: true,
               message: `Pengguna "${form.name}" berhasil diperbarui`,
            }))
         } else {
            res.end(JSON.stringify({
               success: false,
...
exports.delete = async (req, res) => {
   ...
   try {
      ...
      if (decode.logged) {
         db = await conn.getConnection()
         const sql = `call usersDelete(?,?)`
         const result = await db.query(sql, [param.id, decode.user])
         if (result[0][0].status === "ok") {
            res.end(JSON.stringify({
               success: true,
               message: `Pengguna "${send.label}" berhasil dihapus`,
            }))
         } else {
            res.end(JSON.stringify({
               success: false,
